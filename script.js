...
// Загружаем список героев из JSON
...
// Обновляем список героев на странице
function updateHeroList() {
    // Удаляем всех героев из списка
    ...
    for (let i = 0; i < heroes.length; i++) {
        ...
        p.textContent = hero.info;
        const label = document.createElement('label');
        label.textContent = 'Оценка: ';
        const input = document.createElement('input');
        input.type = 'number';
        input.min = '1';
        input.max = '5';

        // Добавляем event listener, чтобы если пользователь введет число больше 5, оно автомаически менялось на 5 
        ...
        // Сохраняем оценку, чтобы при обновлении страницы она осаталась на месте
        input.value = localStorage.getItem(hero.name) || '';
        input.addEventListener('change', function () {
            localStorage.setItem(hero.name, input.value);
        });
